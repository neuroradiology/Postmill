...
function vote($form, isUp) {
    ...
    $.post(url, data).done(() => {
        ...
        $form.find('.vote__button--up').attr('title', getUpButtonTitle(choice));
        $form.find('.vote__button--down').attr('title', getDownButtonTitle(choice));
...
$(function () {
    $(document)
        ...
        .on('click', '.user-logged-in .vote__button', function () {
            ...
            vote($form, $(this).hasClass('vote__button--up'));
        });
});
